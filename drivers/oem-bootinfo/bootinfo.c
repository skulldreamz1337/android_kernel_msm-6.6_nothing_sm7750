...
static ssize_t audio_info_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
	char *s = buf;
	s += sprintf(s, "%s\n",audio_info);
	return (s - buf);
...
static ssize_t audio_info_store(struct kobject *kobj, struct kobj_attribute *attr, const char * buf, size_t n)
{
	return n;
}
static struct kobj_attribute audio_info_attr = {
	.attr = {
		.name = "audio_info",
		.mode = 0644,
	},
	.show =&audio_info_show,
	.store= &audio_info_store,
};
...
{
	unsigned int cable_gpio_num_ant0 = 0;
	unsigned int cable_gpio_num_ant2 = 0;
	int pin_val_ant0 = 0;
	int pin_val_ant2 = 0;
	int ret_ant0 = 0;
	int ret_ant2 = 0;
	...
	//get device node
	node = of_find_matching_node(NULL, cable_of_match);
	if (!node) {
		printk("%s: can't find compatible node\n", __func__);
		return -1;
	}
	cable_gpio_num_ant0 = of_get_named_gpio(node, "cable-gpio-ant0", 0);
	cable_gpio_num_ant2 = of_get_named_gpio(node, "cable-gpio-ant2", 0);
	ret_ant0 = gpio_request_one(cable_gpio_num_ant0, GPIOF_IN, "cable-gpio-ant0");
	if (ret_ant0) {
		printk("%s: Request gpio (%d) failed %d", __func__, cable_gpio_num_ant0, ret_ant0);
		return ret_ant0;
	}
	ret_ant2 = gpio_request_one(cable_gpio_num_ant2, GPIOF_IN, "cable-gpio-ant2");
	if (ret_ant2) {
                printk("%s: Request gpio (%d) failed %d", __func__, cable_gpio_num_ant2, ret_ant2);
                return ret_ant2;
        }

	//ret = gpio_direction_input(cable_gpio_num);
	if (ret_ant0 < 0) {
		printk("%s: gpio-%d input set fail!!!\n", __func__, cable_gpio_num_ant0);
		return ret_ant0;
	}
	if (ret_ant2 < 0) {
                printk("%s: gpio-%d input set fail!!!\n", __func__, cable_gpio_num_ant2);
                return ret_ant2;
        }

	pin_val_ant0 = gpio_get_value(cable_gpio_num_ant0) & 0x01;
	pin_val_ant2 = gpio_get_value(cable_gpio_num_ant2) & 0x01;

	gpio_free(cable_gpio_num_ant0);
	gpio_free(cable_gpio_num_ant2);

	printk(KERN_INFO "%s: cable_gpio[%d] is %d ;\n",__func__, cable_gpio_num_ant0, pin_val_ant0);
	printk(KERN_INFO "%s: cable_gpio[%d] is %d ;\n",__func__, cable_gpio_num_ant2, pin_val_ant2);

	s += sprintf(s, "%d,%d\n",pin_val_ant0,pin_val_ant2);
	return (s - buf);
}
